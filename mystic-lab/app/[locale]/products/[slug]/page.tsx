...
export default async function ProductPage({ params }: Props) {
  ...
  // Auth check는 상품 조회와 독립적으로 실행 — DB 오류가 로그인 상태에 영향주지 않도록
  ...
  try {
    ...
    // Determine which reviewers are verified purchasers
    let purchaserIds = new Set<string>();
    if (reviewData && reviewData.length > 0) {
      try {
        const admin = await createAdminClient();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { data: purchaseRows } = await (admin as any)
          .from("order_items")
        ...
        if (purchaseRows) {
          for (const row of purchaseRows) {
            const uid = (row as { orders: { user_id: string } }).orders?.user_id;
    ...
    reviews = (reviewData ?? []).map((r: any) => ({
      ...(r as ReviewWithProfile),
      verified_purchase: purchaserIds.has(r.user_id as string),
    }));
    ...
    if (currentUserId) {
      const profileRes = await supabase
        ...
        .eq("id", currentUserId)
        .single();
      const profileData = profileRes.data as { role: string; status?: string } | null;
      isAdmin = profileData?.role === "admin";

      // 정지/강퇴 회원은 영상 접근 차단
      const isRestricted =
        profileData?.status === "suspended" || profileData?.status === "banned";
      if (!isRestricted) {
        ...
        if (isAdmin) {
          ...
          // Check manual grant if not already granted via purchase
          if (!hasPurchased) {
