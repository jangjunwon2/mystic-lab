...
export default function ProductDetail({
  ...
  useEffect(() => {
    fetch("/api/products/view", {
      ...
      body: JSON.stringify({ product_id: product.id, locale }),
    }).catch(() => {});
...
function ReviewCard({ review, isLoggedIn }: { review: ReviewWithProfile; isLoggedIn: boolean }) {
  ...
  return (
    <div className="bg-[#1A1A2E] rounded-xl border border-[#2D2D4E] p-5">
      ...
      {isLoggedIn && !reported && (
        <div className="mt-3 pl-12">
          <button
            ...
            disabled={reporting}
            className="text-[11px] text-[#6B7280] hover:text-[#EF4444] transition-colors disabled:opacity-50"
          >
      ...
      {reported && (
        <p className="mt-3 pl-12 text-[11px] text-[#EF4444]">Reported. Thank you.</p>
      )}
    </div>
  );
}
