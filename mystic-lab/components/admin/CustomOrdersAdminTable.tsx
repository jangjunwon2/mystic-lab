...
export default function CustomOrdersAdminTable({ requests: initialRequests }: Props) {
  ...
  async function sendReply(id: string) {
    ...
    if (!subject || !message) {
      setReplyStatus((prev) => ({ ...prev, [id]: "❌ Subject and message required." }));
      return;
    }
    setReplySending(id);
    ...
    if (res.ok) {
      setReplyStatus((prev) => ({ ...prev, [id]: "✅ Email sent." }));
      setReplyMessage((prev) => ({ ...prev, [id]: "" }));
    } else {
      const data = await res.json();
      setReplyStatus((prev) => ({ ...prev, [id]: `❌ ${data.error ?? "Failed to send."}` }));
    }
  }
                      ...
                      <td colSpan={6} className="px-6 py-6 space-y-4">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            >
                              {replySending === req.id ? "Sending…" : "Send Email"}
                            </button>
                            {replyStatus[req.id] && (
                              <span
                                className="text-xs"
