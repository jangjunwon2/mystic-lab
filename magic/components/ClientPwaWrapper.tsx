...
const PWA_TEXTS: Record<string, {
  ...
  ja: {
    ...
    androidDesc: "上の「アプリをインストール」ボタンを押してください。ない場合は右上メニュー(縦3点)→「アプリをインストール」を選びます。⚠️「ホーム画面に追加」(ショートカット)はアドレスバーが残るため、必ず「アプリをインストール」を選んでください。",
  ...
  "zh-CN": {
    ...
    androidDesc: "点击上方“安装应用”按钮。若没有，请点右上角菜单（三个点）→“安装应用”。⚠️“添加到主屏幕”（快捷方式）会保留地址栏，请务必选择“安装应用”。",
  ...
  es: {
    ...
    androidDesc: "Pulse el botón 'Instalar app' de arriba. Si no aparece, abra el menú (tres puntos) → 'Instalar aplicación'. ⚠️ 'Añadir a la pantalla de inicio' (acceso directo) deja la barra de direcciones; elija siempre 'Instalar aplicación'.",
  ...
  fr: {
    ...
    androidDesc: "Appuyez sur le bouton 'Installer l'app' ci-dessus. Sinon, menu (trois points) → 'Installer l'application'. ⚠️ 'Ajouter à l'écran d'accueil' (raccourci) garde la barre d'adresse ; choisissez toujours 'Installer l'application'.",
  ...
  de: {
    ...
    androidDesc: "Tippen Sie oben auf 'App installieren'. Falls nicht vorhanden: Menü (drei Punkte) → 'App installieren'. ⚠️ 'Zum Startbildschirm hinzufügen' (Verknüpfung) behält die Adressleiste — wählen Sie immer 'App installieren'.",
