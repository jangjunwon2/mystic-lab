...
export interface WishlistCoupon {
  enabled: boolean;
  days: number;
  percent: number;
  months: number;
...
// 위시리스트 트리거 쿠폰 — N일+ 위시리스트 잔존 상품에 대해 해당 회원에게 그 상품 한정 쿠폰 자동발급(일 1회 cron).
export default function WishlistCouponCard({ initial }: { initial: WishlistCoupon }) {
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/settings", {
      ...
      body: JSON.stringify({ wishlistCoupon: { enabled, days: d, percent: p, months: m, name: name.trim() } }),
    });
    setSaving(false);
  ...
  return (
    <div className="rounded-xl border p-6 max-w-md" style={cardStyle}>
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-base font-semibold" style={{ color: "#F0E6FF" }}>위시리스트 정기 쿠폰</h2>
      ...
      <p className="text-xs mb-4" style={{ color: "#9CA3AF" }}>
        위시리스트에 일정 기간 담아둔 상품에 대해, 해당 회원에게 <b>그 상품 한정 쿠폰</b>을 자동 발급(일 1회). 재구매 유도용.
      </p>
      <div className="space-y-3">
        <div>
          <label className="block text-xs mb-1" style={{ color: "#9CA3AF" }}>쿠폰 이름(고객 표시)</label>
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} maxLength={60} placeholder="위시리스트 특별 할인"
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm" style={{ color: "#9CA3AF" }}>일 잔존</span>
          </div>
          <div className="flex items-center gap-1.5">
