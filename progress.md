...
- **쿠폰 배포 2방식** ✅: ① **회원 직접 할당**(개인 쿠폰 — 어드민이 회원에게 발급, 발급 페이지 없이 계정에 자동) ② **전용 발급 페이지**(공개 쿠폰 — `/[locale]/coupons`에서 고객이 직접 발급/claim받아야 사용). 공개 쿠폰은 `coupon_claims`(회원당 1회) 기록이 있어야 validate 통과(코드만 입력해선 사용 불가). `/api/account/coupons`가 발급받은 공개+개인(드롭다운) / 발급가능 공개(claimable) 분리 반환, `POST .../claim`. 마이페이지 "내 쿠폰" 탭은 **보유 쿠폰만** + 발급 가능분은 **발급 페이지 링크**로 이동. ⚠️ **`044_coupon_claims.sql`** 필요
...
- **OTA 흐름**: 기기 → `/api/firmware/latest?device=X` JSON(`version`, `url`, `notes`) → 서버 버전이 현재보다 높을 때만(`isVersionNewer`) 저장된 URL로 바이너리 다운로드 → `Update.h` 적용 → 재부팅
  - Vercel chunked 전송: `http.getStream()` → `http.getString()` 변경(JSON 파싱 깨짐 수정) — 5종 전체 적용
  - `_otaFirmwareUrl` 멤버 변수(pot·flux·receiver·smoke) / `otaState.firmwareUrl`(transmitter)로 version check → download URL 전달
- **버전 관리**: `config.h`(또는 transmitter는 `config_t.h`)의 `FIRMWARE_VERSION` 단일 소스 — `#define` 및 `constexpr const char*` 형식 모두 지원. 형식은 `"major.minor"` (예: `"1.0"`, `"1.1"`). `version.txt` 사용 안 함
- **버전 비교**: `isVersionNewer()`(시맨틱) — 서버 버전이 엄격히 높을 때만 업데이트. 동일·낮은 버전은 무시(다운그레이드 불가)
...
- [x] **마이그레이션 `046_product_point_rate.sql` 실행 완료** — 상품별 포인트 적립률 오버라이드 컬럼 추가.
...
## 📐 쿠폰 통합 + 프로모션 엔진 — Phase 0~C3 + Phase 2 구현 완료 (잔여: 프로모션 대시보드)
...
**✅ Phase 2 — 이벤트 일괄발급** (구현 완료, 라이브 점검 대기): `POST /api/admin/coupons/bulk` — 대상 5종(전체회원·뉴스레터구독자·과거구매자·위시리스트상품지정·직접입력) 해석 → 중복가드(이메일) → 상한 2000 → `issueCoupon(source='bulk')` 루프(1인 1코드·1회용) → 안내이메일 100통씩 배치(`sendCouponEmailsBatch`, `resend.batch.send`). 어드민 `/admin/coupons` 상단 "이벤트 일괄 발급" 카드(대상 셀렉트·조건부 상품/이메일 입력·쿠폰 사양·상품한정·이메일 토글). 발급분은 개인쿠폰 모델이라 마이페이지 "내 쿠폰"·결제 검증 그대로(회귀 없음·새 마이그레이션 불필요). `maxDuration=300`
