...
- 장바구니(**부분 선택 결제**) · 즉시구매 · **할인·레퍼럴·개인 쿠폰**(공용 입력창) · **마일리지 사용/적립**
...
- **쿠폰 1인당 한도 C2a** ✅ (#2-C2a): `per_user_limit` + `coupon_redemptions`(사용이력) — save-order가 사용 시 이력 기록, validate가 1인당 한도 enforce(로그인 회원). 어드민 공개쿠폰 폼에 "1인당 사용 한도" 입력. ⚠️ **`041_coupon_per_user_limit.sql`** 필요. **남은 #2**: C2b(상품/카테고리 한정=해당상품 소계할인) · C3(트리거: 위시리스트 먼저, 장바구니는 서버동기화 선행)
