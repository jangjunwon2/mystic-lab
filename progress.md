...
> **✅ 처리 결과 (2026-06-05, 커밋 `7f20768`·`d61d163`)** — A·B·C(reviews)·D(사용자향)+뉴스레터 iframe·G(매뉴얼 동기화) **수정·배포 완료**. `npx tsc` 통과. **남은 항목(의도적 보류)**: D-어드민 error.message(어드민 전용·신뢰 사용자, 저위험), C-업로드 매직바이트(어드민 전용), G-계산기 eval→파서/설정 PIN(기능추가·저위험), I-`as any`(대규모 리팩터·런타임 정상), F-`033` 실행(운영자), 인스타 iOS 실기기 확인(배포됨).
