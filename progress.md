...
### 어드민 (/admin · 한국어)
- 대시보드 · 통계(매출·전환율·위시→구매·공유·결제수단/국가 분포·**기간 지정 CSV 내보내기**)
...
- **7개 언어**: 게시물 caption/date/댓글(LocalizedText) + 좋아요 문구·빈 상태·DM 라벨 + **비밀 설정창 전체**. 정확한 게시일·기기 뒤로가기 단계 복귀
...
- **통계 대시보드 고도화** — 고객 세그먼트 분석 등 (기간 지정 CSV 내보내기는 완료)
