...
- 배송 관리 전용 페이지 · 리뷰(게시 관리) · 커스텀의뢰 · 할인 · 영상 · 공지(**프로모션 쿠폰코드** 표시) · 뉴스레터
...
- [ ] **Supabase 마이그레이션 `035_coupons.sql`** 실행 — 쿠폰 시스템(issued_coupons + 레퍼럴 보상/할인타입 + 공지 쿠폰코드). 미실행 시 쿠폰 발급·개인쿠폰·레퍼럴 보상 동작 안 함
