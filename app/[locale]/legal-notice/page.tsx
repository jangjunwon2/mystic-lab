...
const UPDATED: Record<string, string> = { ko: "최종 수정일", ja: "最終更新일", "zh-CN": "最后更新", de: "Zuletzt aktualisiert", fr: "Dernière mise à jour", es: "Última actualización", en: "Last updated" };
...
export default async function LegalNoticePage({ params }: Props) {
  ...
  const BIZ_ADDRESS = await getSetting(supabase, "biz_address", process.env.BIZ_ADDRESS || "[사업장 주소 기입]");
  const BIZ_PHONE = await getSetting(supabase, "biz_phone", process.env.BIZ_PHONE || "[연락처 기입]");
  const BIZ_EMAIL = await getSetting(supabase, "biz_email", process.env.BIZ_EMAIL || "jun923008@gmail.com");
  const BIZ_COMMUNICATION_REG = await getSetting(supabase, "biz_communication_reg", process.env.BIZ_COMMUNICATION_REG || "[통신판매업 신고번호 기입]");
  ...
  const content: Record<string, LegalContent> = {
    ...
    ja: {
      ...
      sections: [
        ...
        { heading: "支払方法・支払時期", content: "クレジットカード等(LemonSqueezy / Toss Payments)。注文时にお支払いいただきます。" },
