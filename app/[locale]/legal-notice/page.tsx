...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const titles: Record<string, string> = {
    en: "Legal Notice | Mystic Lab",
    ko: "법적 고지 | 미스틱 랩",
    ja: "法的通知 | ミスティック・ラボ",
    "zh-CN": "法律声明 | 神秘实验室",
    es: "Aviso legal | Mystic Lab",
    fr: "Mentions légales | Mystic Lab",
    de: "Impressum | Mystic Lab",
  };
  return {
    title: titles[locale] ?? titles.en,
    alternates: {
      canonical: `${SITE_URL}/${locale}/legal-notice`,
      languages: {
        ...Object.fromEntries(LOCALES.map((l) => [l, `${SITE_URL}/${l}/legal-notice`])),
...
// 실제 제공된 사업자 정보. 환경 변수로 오버라이드 가능하도록 보완.
const BIZ_NAME = process.env.BIZ_NAME || "비에이블 (Beable)";
const BIZ_REG = process.env.BIZ_REG || "742-10-02095";
const BIZ_REPRESENTATIVE = process.env.BIZ_REPRESENTATIVE || "장준원";
const BIZ_ADDRESS = process.env.BIZ_ADDRESS || "[사업장 주소 기입]";
const BIZ_PHONE = process.env.BIZ_PHONE || "[연락처 기입]";
const BIZ_EMAIL = process.env.BIZ_EMAIL || "jun923008@gmail.com";
const BIZ_COMMUNICATION_REG = process.env.BIZ_COMMUNICATION_REG || "[통신판매업 신고번호 기입]";
const BIZ_PRIVACY_OFFICER = process.env.BIZ_PRIVACY_OFFICER || "장준원 (jun923008@gmail.com)";
...
const content: Record<string, LegalContent> = {
  ko: {
    ...
    sections: [
      ...
      { heading: "대표자", content: `대표자: ${BIZ_REPRESENTATIVE}` },
      { heading: "통신판매업 신고번호", content: `${BIZ_COMMUNICATION_REG} (관할 시·군·구청 신고)` },
      { heading: "사업장 주소", content: BIZ_ADDRESS },
      { heading: "연락처", content: `전화: ${BIZ_PHONE} · 이메일: ${BIZ_EMAIL}` },
      { heading: "개인정보보호책임자", content: BIZ_PRIVACY_OFFICER },
  ...
  ja: {
    ...
    sections: [
      { heading: "販売業者", content: `${BIZ_NAME} · 韓国事業者登録番号: ${BIZ_REG}` },
      { heading: "運営統括責任者", content: BIZ_REPRESENTATIVE },
      { heading: "所在地", content: BIZ_ADDRESS },
      { heading: "連絡先", content: `電話: ${BIZ_PHONE} · メール: ${BIZ_EMAIL}` },
      { heading: "販売価格", content: "各商品ページに表示する価格(税込)。" },
      { heading: "商品代金以外の必要料金", content: "送料(配送방법により異なる)、関税・輸入消費税(該当する場合は購入者負担)。" },
  ...
  de: {
    ...
    sections: [
      { heading: "Anbieter", content: `${BIZ_NAME} · Geschäftsregisternummer (Korea): ${BIZ_REG}` },
      { heading: "Vertreten durch", content: BIZ_REPRESENTATIVE },
      { heading: "Anschrift", content: BIZ_ADDRESS },
      { heading: "Kontakt", content: `Telefon: ${BIZ_PHONE} · E-Mail: ${BIZ_EMAIL}` },
  ...
  en: {
    ...
    sections: [
      { heading: "Seller", content: `${BIZ_NAME} · Business registration no. (Korea): ${BIZ_REG}` },
      { heading: "Representative", content: BIZ_REPRESENTATIVE },
      { heading: "Registered address", content: BIZ_ADDRESS },
      { heading: "Contact", content: `Phone: ${BIZ_PHONE} · Email: ${BIZ_EMAIL}` },
  ...
  es: {
    ...
    sections: [
      { heading: "Vendedor", content: `${BIZ_NAME} · N.º de registro mercantil (Corea): ${BIZ_REG}` },
      { heading: "Representante", content: BIZ_REPRESENTATIVE },
      { heading: "Domicilio", content: BIZ_ADDRESS },
      { heading: "Contacto", content: `Teléfono: ${BIZ_PHONE} · Correo: ${BIZ_EMAIL}` },
  ...
  fr: {
    ...
    sections: [
      { heading: "Vendeur", content: `${BIZ_NAME} · N° d'enregistrement (Corée) : ${BIZ_REG}` },
      { heading: "Représentant", content: BIZ_REPRESENTATIVE },
      { heading: "Adresse", content: BIZ_ADDRESS },
      { heading: "Contact", content: `Téléphone : ${BIZ_PHONE} · E-mail : ${BIZ_EMAIL}` },
  ...
  "zh-CN": {
    ...
    intro: "卖家及经营者信息。",
    sections: [
      { heading: "销售商", content: `${BIZ_NAME} · 韩国营业执照号: ${BIZ_REG}` },
      { heading: "负责人", content: BIZ_REPRESENTATIVE },
      { heading: "营业地址", content: BIZ_ADDRESS },
      { heading: "联系方式", content: `电话: ${BIZ_PHONE} · 邮箱: ${BIZ_EMAIL}` },
